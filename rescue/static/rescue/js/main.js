...
const world = { width: canvas.width, height: canvas.height, keys: {}, playing: false, difficulty: 'Curiosa', scene: 'Delta al atardecer', tick: 0 };
...
function resetGame() {
  ...
  enemies = Array.from({ length: hard ? 10 : 7 }, (_, index) => ({ x: 170 + (index * 107) % 670, y: 310 + (index * 71) % 180, radius: index % 3 === 0 ? 24 : 20, speed: random(.7, 1.25) * (hard ? 1.2 : 1), direction: index % 2 ? 1 : -1, kind: index % 3 === 0 ? 'crab' : 'eel' }));
  currents = hard
  ...
  world.tick = 0; world.playing = true;
...
function update() {
  if (!world.playing) return; world.tick += 1; const speed = world.difficulty === 'Guardiana' ? 3.15 : 4;
  ...
  enemies.forEach((item) => { item.x += item.speed * item.direction; if (item.x < 45 || item.x > 915) item.direction *= -1; item.y += Math.sin((world.tick + item.x) / 35) * .3; if (player.invincible <= 0 && distance(player, item) < player.radius + item.radius) { player.health -= 1; player.invincible = 80; updateHud(`¡Un ${item.kind === 'crab' ? 'cangrejo invasor' : 'pez invasor'} te bloqueó el paso!`); if (!player.health) { world.playing = false; gameMessage.textContent = 'La nutria necesita descansar. Tocá Reiniciar para intentarlo de nuevo.'; } } });
...
function loop() { drawBackground(); currents.forEach(drawCurrent); drawHome(); trash.filter((item) => !item.taken).forEach(drawTrash); shells.filter((item) => !item.taken).forEach(drawShell); enemies.forEach(drawEnemy); drawOtter(); update(); animation = requestAnimationFrame(loop); }
...
window.addEventListener('keydown', (event) => { if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'w', 'a', 's', 'd'].includes(event.key)) { world.keys[event.key] = true; event.preventDefault(); } }); window.addEventListener('keyup', (event) => { world.keys[event.key] = false; });
...
document.querySelector('#restart-game').addEventListener('click', beginGame); gameForm.addEventListener('submit', (event) => { event.preventDefault(); const selections = new FormData(gameForm); world.difficulty = selections.get('difficulty'); world.scene = selections.get('scene'); gameSummary.textContent = `${selections.get('mode')}, ${world.scene}. Dificultad: ${world.difficulty}.`; game.hidden = false; modal.close(); game.scrollIntoView({ behavior: 'smooth', block: 'start' }); game.focus({ preventScroll: true }); beginGame(); }); updateBestScore();
