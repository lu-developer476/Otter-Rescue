...
function clearMovement() { ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'w', 'a', 's', 'd'].forEach((key) => { world.keys[key] = false; }); }
function setCanvasDirection(deltaX, deltaY) {
  clearMovement();
  if (Math.abs(deltaX) < 12 && Math.abs(deltaY) < 12) return;
  world.keys[Math.abs(deltaX) > Math.abs(deltaY) ? (deltaX > 0 ? 'ArrowRight' : 'ArrowLeft') : (deltaY > 0 ? 'ArrowDown' : 'ArrowUp')] = true;
}
...
window.addEventListener('keydown', (event) => { const key = event.key.length === 1 ? event.key.toLowerCase() : event.key; if (key === 'p' && !event.repeat) { togglePause(); event.preventDefault(); } if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'w', 'a', 's', 'd'].includes(key)) { world.keys[key] = true; event.preventDefault(); } }); window.addEventListener('keyup', (event) => { world.keys[event.key.length === 1 ? event.key.toLowerCase() : event.key] = false; }); window.addEventListener('blur', clearMovement);
document.querySelectorAll('[data-direction]').forEach((button) => { const key = ({ up: 'ArrowUp', down: 'ArrowDown', left: 'ArrowLeft', right: 'ArrowRight' })[button.dataset.direction]; const pointers = new Set(); const release = (event) => { pointers.delete(event.pointerId); world.keys[key] = pointers.size > 0; event.preventDefault(); }; button.addEventListener('pointerdown', (event) => { pointers.add(event.pointerId); button.setPointerCapture(event.pointerId); world.keys[key] = true; event.preventDefault(); }); ['pointerup', 'pointercancel', 'lostpointercapture'].forEach((eventName) => button.addEventListener(eventName, release)); });
let canvasTouch;
canvas.addEventListener('pointerdown', (event) => { if (event.pointerType === 'mouse') return; canvasTouch = { id: event.pointerId, x: event.clientX, y: event.clientY }; canvas.setPointerCapture(event.pointerId); event.preventDefault(); });
canvas.addEventListener('pointermove', (event) => { if (!canvasTouch || canvasTouch.id !== event.pointerId) return; setCanvasDirection(event.clientX - canvasTouch.x, event.clientY - canvasTouch.y); event.preventDefault(); });
['pointerup', 'pointercancel', 'lostpointercapture'].forEach((eventName) => canvas.addEventListener(eventName, (event) => { if (canvasTouch && canvasTouch.id === event.pointerId) { canvasTouch = null; clearMovement(); } event.preventDefault(); }));
