...
function spawnDefence() {
  const level = 1 + Math.floor(world.tick / 600); if (level > world.level) { world.level = level; updateHud(`¡Oleada ${level}! El río acelera sus desafíos.`); }
...
function hit(message){if(player.invincible>0||player.shield>0)return;player.health--;player.invincible=80;updateHud(message);if(!player.health)endGame('La nutria necesita descansar. Tocá Reiniciar para intentarlo de nuevo.');}
...
function finishMission(){const bonus=player.shells===shells.length?100:0;player.score+=bonus;hud.goal.textContent=bonus?'¡Colección completa! +100 puntos':'Volvé a explorar para encontrar las conchas';endGame(`¡Misión cumplida! ${bonus?'¡Bonus de exploradora!':'La nutria volvió con su familia.'} 🦦`);}
function updateExplore(){const level=1+Math.floor(world.tick/720);if(level>world.level){world.level=level;enemies.push(roamingEnemy(enemies.length+level));updateHud(`¡El humedal cambia! Nivel de desafío ${level}.`);}currents.forEach(c=>{if(distance(player,c)<c.radius){player.x+=c.dx;player.y+=c.dy;}});trash.forEach(i=>{if(!i.taken&&distance(player,i)<player.radius+i.radius){i.taken=true;player.collected++;player.score+=20;updateHud('¡Residuo rescatado! +20 puntos');}});shells.forEach(i=>{if(!i.taken&&distance(player,i)<player.radius+i.radius){i.taken=true;player.shells++;player.score+=15;updateHud('¡Concha encontrada! +15 puntos');}});enemies.forEach(i=>{i.x+=i.speed*(1+(world.level-1)*.12)*i.direction;i.y+=Math.sin((world.tick+i.x)/25)*.55;if(i.x<35||i.x>925)i.direction*=-1;if(distance(player,i)<player.radius+i.radius)hit('¡Un invasor te bloqueó el paso!');});if(player.collected===trash.length&&distance(player,home)<player.radius+home.radius)finishMission();}
function updateDefence(){spawnDefence(); enemies.forEach(i=>{i.x-=i.speed;i.y+=Math.sin((world.tick+i.wave)/18)*.7;if(distance(player,i)<player.radius+i.radius){i.x=1000;player.score+=12;dam.branches++;updateHud('¡Atajaste una amenaza! +12 puntos');}if(i.x<i.radius+72){i.x=1000;dam.health-=i.kind==='beaver'?18:11;updateHud('¡La represa recibió un golpe!');}});bonuses.forEach(i=>{i.x-=i.speed;if(distance(player,i)<player.radius+i.radius){i.x=-40;if(i.kind==='branch'){dam.branches++;dam.health=Math.min(100,dam.health+12);player.score+=18;updateHud('¡Rama entregada! Represa reparada.');}else{player.health=Math.min(3,player.health+1);player.shield=220;updateHud('¡Corazón de río! Escudo temporal.');}}});enemies=enemies.filter(i=>i.x>-50&&i.x<1030);bonuses=bonuses.filter(i=>i.x>-40);if(dam.health<=0)endGame('La represa cedió. Reuní ramas y atajá las amenazas más temprano.');}
...
function loop(){render();update();animation=requestAnimationFrame(loop);} function beginGame(){cancelAnimationFrame(animation);resetGame();loop();}
function togglePause(){if(!world.playing)return;world.paused=!world.paused;pauseButton.textContent=world.paused?'▶ Reanudar':'Ⅱ Pausar';pauseButton.setAttribute('aria-pressed',String(world.paused));hud.message.textContent=world.paused?'Partida pausada.':'¡Seguimos!';}
